...
test("Sol is preferred only when visible in the client's available models", async () => {
  assert.equal(await createModelResolver(async () => [{model:"gpt-5.6-sol"}])("client"), "gpt-5.6-sol");
  assert.equal(await createModelResolver(async () => [{model:"gpt-5.6-sol",hidden:true}])("client"), undefined);
...
test("parallel jobs share discovery; catalog and failures expire for subsequent jobs", async () => {
  ...
  const resolve=createModelResolver(async () => {calls++;if(fail)throw new Error("offline");return [{model:"gpt-5.6-sol"}];},()=>now);
