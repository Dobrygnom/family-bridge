...
import { contextSourceHash, COVERAGE_RECOVERY_VERSION } from "../src/core/context-analysis.js";
  ...
  test(`saved coverage failure recovery: ${scenario}`, async () => {
    ...
    try {
      ...
      await writeFile(path.join(memory, "context-analysis.json"), JSON.stringify({
        ...
        coverageRecoveryAttempted: true, // A failed repair in the old release must not block the fix.
      }));
      ...
      service.analyzeContext = async (id: string, hash: string, cached: unknown, previous: any) => {
        ...
        assert.equal(previous.coverageRecoveryVersion, COVERAGE_RECOVERY_VERSION);
