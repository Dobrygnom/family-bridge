...
const labels = {
  ru: { current:"Сейчас установлена", check:"Проверить обновления", checking:"Проверяем обновления…", download:"Скачиваем обновление", ready:"Обновление готово", install:"Обновить сейчас", installing:"Устанавливаем обновление…", requested:"Обновление запущено", auto:"Обновление установится автоматически. При желании можно запустить его сейчас.", restart:"Приложение перезапустится. Все разговоры и черновики сохранятся.", dictation:"Сначала завершите запись голоса. Затем обновление продолжится автоматически.", editing:"Сначала закончите текущее редактирование. Затем обновление продолжится автоматически.", activity:"Завершаем текущее действие. Затем обновление продолжится автоматически.", background:"Сохраняем данные и завершаем текущий ответ. Затем обновление продолжится автоматически.", error:"Не удалось установить обновление. Повторите попытку." },
...
export function UpdateControl({ update, version, language, onCheck, onInstall, compact = false }: {
  ...
  if (compact) {
    ...
    const installPending = update.ready && busy && !error && !update.error;
