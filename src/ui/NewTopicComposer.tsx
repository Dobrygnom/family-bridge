...
const labels = {
  ru: { add: "Новая тема", heading: "Начать новый разговор", agent: "Помощник подготовит", direct: "Отправить мои слова", description: "Что произошло и о чём хочется поговорить", private: "Это описание нужно только для подготовки. Перед отправкой вы увидите точный текст.", prepare: "Подготовить реплику", preparing: "Готовим черновик…", title: "Название разговора", message: "Первая реплика", preview: "Собеседник получит именно этот текст", instruction: "Что изменить или уточнить", refine: "Переписать", send: "Отправить", sending: "Сохраняем и отправляем…", saved: "Черновик сохранён", queued: "Реплика сохранена. Разговор начнётся автоматически, когда связь будет доступна.", close: "Закрыть", missing: "Сначала подключите собеседника", changed: "Описание изменилось. Подготовьте реплику ещё раз, чтобы учесть изменения.", directHint: "Напишите реплику своими словами. Она не будет изменена перед отправкой.", next: "После первой реплики помощники продолжат разговор.", failure: "Не удалось сохранить черновик. Текст остаётся в форме.", empty: "Опишите ситуацию, важные обстоятельства, свои мысли и вопросы.", recipient: "Собеседник", previewTitle: "Реплика для отправки" },
...
export function NewTopicComposer({ state, language, onState, onActive }: {
  ...
  const sharedLabel = { ru: "Пояснение для помощника собеседника", en: "Note for your partner’s assistant", cs: "Poznámka pro pomocníka partnera", fr: "Note pour l’assistant du partenaire" }[language];
  const sharedHint = { ru: "Пояснение поможет подготовить ответ, но в самом разговоре будет видна только реплика.", en: "The note helps prepare a reply, but only the message appears in the conversation.", cs: "Poznámka pomůže připravit odpověď, ale v rozhovoru se zobrazí jen zpráva.", fr: "La note aide à préparer la réponse, mais seul le message apparaît dans la conversation." }[language];
