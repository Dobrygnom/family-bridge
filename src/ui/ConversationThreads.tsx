      ...
      return <article className={`report-card ${selected ? "selected-report" : ""}`} key={thread.id} data-thread-id={thread.id}>
        <StableDetails className="conversation-thread" storageKey={`family-bridge-thread-open:${thread.id}`} revealKey={selected ? selectedReportId : ""}>
          <summary className="report-heading"><strong>{thread.topic}</strong><span className="report-status">
            <span>{repairPending ? repairLabel : thread.live ? t.live : thread.latest?.completionState === "needs_follow_up" ? t.follow : t.done}</span>
          ...
          {repairPending && <p className="muted" role="status">{repairHint}</p>}
          {!repairPending && thread.currentStages.map((stage, index) => <section id={`report-${stage.id}`} className="conversation-stage" key={stage.id}>
            {(index > 0 || stage.restarted) && <div className="conversation-updates-heading"><strong>{stage.restarted ? restartLabel : t.next}</strong><small>{stage.live ? t.live : stage.report?.completedAt ? new Date(stage.report.completedAt).toLocaleString(language) : ""}</small></div>}
