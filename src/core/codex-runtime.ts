...
- не пиши «оба участника могут», «мы получили содержательный результат», «стороны договорились» и подобные отчётные формулировки. Используй имена только из настроек и пиши так, как эти два человека разговаривали бы друг с другом сами;
...
- shared_summary: максимум 240 символов и 1–2 живых предложения. Никакого нейтрального резюме, рекомендаций участникам в третьем лице или пересказа переговоров;
...
- не ставь разговор на паузу ради примера, точной формулировки, определения слова или вопроса «правильно ли я понял», если суть можно обсуждать без них. Необязательную деталь оставь неизвестной. Не выбирай за человека неизвестную дату, срок, частоту, личное решение или подтверждение готовности; автономия не разрешает выдумывать факты. Если именно такой факт необходим, правило paused важнее требования continue для первого ответа;
...
export function buildInitialPrompt(options: CodexRuntimeOptions, initialPrompt: string): string {
  const ownerName = options.ownerName ?? options.displayName;
  const peerName = options.peerName ?? "собеседник";
...
Не спрашивай владельца ради примера, точной формулировки, определения или проверки «правильно ли я понял», если можно обсуждать суть без них. Неизвестные даты, сроки, числа, намерения и готовность не выбирай самостоятельно. Нельзя снять паузу, если для этого придётся выдумать личный факт. Изложи известную общую позицию без объяснения неизвестного эпизода, если этого достаточно для продолжения.
${agentKnowledgeRules}
...
export class CodexCliAgent implements AgentRuntime {
  ...
  async respond(peerMessage: string, guidance = ""): Promise<AgentResponse> {
    return this.resume(`Сообщение второго личного агента:\n${peerMessage}\n\nОтветь по существу живой репликой в стиле владельца. Не превращай ответ в психологический протокол. Соблюдай роль инициатора или отвечающей стороны, заданную в начале сессии.\n${agentKnowledgeRules}${guidance ? `\n\nВнутренняя инструкция продолжения (это не слова собеседника):\n${guidance}` : ""}\n\nВерни только JSON.`);
  }
  ...
  private async enforceOutput(initial: AgentResponse): Promise<AgentResponse> {
    const ownerName = this.options.ownerName ?? this.options.displayName;
    const peerName = this.options.peerName ?? "собеседник";
