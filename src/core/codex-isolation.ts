...
export function codexTaskFailure(task: string, code: number | null, output: string): Error {
  ...
  if (/usage limit|rate limit|quota|too many requests|limit (?:has been )?reached|try again (?:at|later)|лимит.*исчерпан/i.test(output)) {
    return Object.assign(new Error("Лимит Codex временно исчерпан. Задание сохранено и будет повторено автоматически."), { code: "CODEX_USAGE_LIMIT" });
  }
