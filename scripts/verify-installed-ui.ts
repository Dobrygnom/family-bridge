...
assert.equal(appSummary.portraits, appSummary.people + 1, "Portraits do not cover the owner and every known person");
assert.equal(appSummary.ownerPortraits, 1, "The owner portrait is missing or duplicated");
assert.ok(appSummary.observations > 0, "No portrait observations were created");
