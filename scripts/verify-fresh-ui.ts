...
assert.match(mainText, /Начать без существующего чата/);
...
assert.match(updateText, /Установлена версия 2\.0\.0/);
const settingsText = await evaluate<string>(`document.querySelector('main')?.innerText ?? ''`);
assert.match(settingsText, /Вычислительный канал/);
assert.match(settingsText, /Этот компьютер помогает/);
assert.match(settingsText, /Подключить помощника/);
assert.match(settingsText, /Автоматически \(по аккаунту\)/);
console.log(JSON.stringify({ freshProfile: true, navigation, firstActions: ["choose-chat", "manual-profile"], overflow, updateStatus: true, computeSwitch: true }));
socket.close();
